...
namespace sdsl
{
	...
	template < class t_bitvector = bit_vector,
		class t_succ_tree = bp_support_sada < >, 		class t_rank = typename t_bitvector::rank_1_type,
	  	 ...
	  	 spanning tree */
			...
			void swap(pemb & g)
			{
				if (this != &g)
				{
					...
					util::swap_support(m_B_star_st, g.m_B_star_st, &m_B_star, &(g.m_B_star));
				}
			}
			...
			size_type pemb_faces() const
			{
				return m_edges - m_vertices + 2;
			}
			...
			/*Get the first edge of the vertex x (in the primal graph) */
			...
			/*Assuming indices start with 0 */
			/*Get the first edge of the face f (in the dual graph) */
			size_type first_dual(size_type f)
			{
				if (f >= 0)
				{
					...
					if (f == 0)	// The root of the dual spanning tree
						return edge;
					else
						return edge + 1;
				}
